...
void PlayerbotDbStore::Load(PlayerbotAI* botAI)
{
    ...
    QueryResult results = PlayerbotDatabase.PQuery("SELECT `key`,`value` FROM `playerbot_db_store` WHERE `guid` = '%u'", guid);
    if (results)
    {
...
void PlayerbotDbStore::Reset(PlayerbotAI* botAI)
{
    ...
    PlayerbotDatabase.PExecute("DELETE FROM `playerbot_db_store` WHERE `guid` = '%u'", guid);
}

void PlayerbotDbStore::SaveValue(uint32 guid, std::string const& key, std::string const& value)
{
    PlayerbotDatabase.PExecute("INSERT INTO `playerbot_db_store` (`guid`, `key`, `value`) VALUES ('%u', '%s', '%s')", guid, key.c_str(), value.c_str());
}
