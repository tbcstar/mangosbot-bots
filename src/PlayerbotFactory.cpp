...
PlayerbotFactory::PlayerbotFactory(Player* bot, uint32 level, uint32 itemQuality = 0) : level(level), itemQuality(itemQuality), InventoryAction(bot->GetPlayerbotAI(), "factory")
{
}
...
void PlayerbotFactory::ClearSpells()
{
    ...
    for (std::vector<uint32>::iterator i = spells.begin(); i != spells.end(); ++i)
    {
...
bool PlayerbotFactory::IsDesiredReplacement(Item* item)
{
    ...
    return bot->getLevel() - proto->RequiredLevel > delta;
}
...
void PlayerbotFactory::InitSecondEquipmentSet()
{
    ...
    for (std::map<uint32, std::vector<uint32>>::iterator i = items.begin(); i != items.end(); ++i)
    {
        ...
        for (uint32 attempts = 0; attempts < 15; attempts++)
        {
            ...
            if (Item* newItem = bot->StoreNewItemInInventorySlot(newItemId, 1))
            {
                EnchantItem(newItem);
                newItem->AddToWorld();
                newItem->AddToUpdateQueueOf(bot);
...
void PlayerbotFactory::InitAvailableSpells()
{
    ...
    for (auto const& itr : *creatureTemplates)
    {
        ...
        TrainerSpellData const* trainer_spells = sObjectMgr->GetNpcTrainerSpells(co->Entry);
        if (!trainer_spells)
            continue;
		...
		for (TrainerSpellMap::const_iterator itr = trainer_spells->spellList.begin(); itr != trainer_spells->spellList.end(); ++itr)
		{
			...
			uint32 reqLevel = tSpell->reqLevel;
...
void PlayerbotFactory::InitAmmo()
{
    ...
    if (count < maxCount)
    {
        ...
        {
            if (Item* newItem = bot->StoreNewItemInInventorySlot(entry, 200))
                newItem->AddToUpdateQueueOf(bot);
        }
    }
...
{
    std::map<uint32, std::vector<uint32>> spells;
    ...
    for (uint32 spellId = 0; spellId < sSpellMgr->GetSpellInfoStoreSize(); ++spellId)
    {
        ...
        if (spellInfo->CalcCastTime() < 500 || spellInfo->GetDuration() != -1)
            continue;
    ...
    {
        for (std::map<uint32, std::vector<uint32> >::iterator i = spells.begin(); i != spells.end(); ++i)
        {
            uint32 effect = i->first;
            std::vector<uint32>& ids = i->second;
            uint32 index = urand(0, ids.size() - 1);
            if (index >= ids.size())
                continue;
...
void PlayerbotFactory::InitPotions()
{
    ...
    for (uint8 i = 0; i < 2; ++i)
    {
        ...
        if (Item* newItem = bot->StoreNewItemInInventorySlot(itemId, urand(maxCount / 2, maxCount)))
            newItem->AddToUpdateQueueOf(bot);
   }
}
