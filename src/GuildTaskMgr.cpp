...
void GuildTaskMgr::Update(Player* player, Player* guildMaster)
{
    ...
    {
        LOG_DEBUG("playerbots", "%s / %s: skipping guild task update - not enough security level, reason = %u", guild->GetName().c_str(), player->GetName().c_str(), reason);
        return;
    }
    ...
    if (!activeTask)
    {
        ...
        {
            LOG_ERROR("playerbots",  "%s / %s: error creating guild task", guild->GetName().c_str(), player->GetName().c_str());
        }
        ...
        LOG_DEBUG("playerbots", "%s / %s: guild task %u is set for %u secs", guild->GetName().c_str(), player->GetName().c_str(), task, time);
        return;
    }
    ...
    if (!advertisement)
    {
        ...
        {
            LOG_ERROR("playerbots",  "%s / %s: error sending advertisement", guild->GetName().c_str(), player->GetName().c_str());
        }
    }
    ...
    if (!thanks)
    {
        ...
        {
            LOG_ERROR("playerbots",  "%s / %s: error sending thanks", guild->GetName().c_str(), player->GetName().c_str());
        }
    }
    ...
    if (!reward)
    {
        ...
        {
            LOG_ERROR("playerbots",  "%s / %s: error sending reward", guild->GetName().c_str(), player->GetName().c_str());
        }
    }
...
bool GuildTaskMgr::CreateItemTask(Player* player, uint32 guildId)
{
    ...
    if (!itemId)
    {
        LOG_ERROR("playerbots",  "%s / %s: no items avaible for item task", sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str());
        return false;
    }

    uint32 count = GetMaxItemTaskCount(itemId);

    LOG_DEBUG("playerbots", "%s / %s: item task %u (x%d)", sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str(), itemId, count);
...
bool GuildTaskMgr::CreateKillTask(Player* player, uint32 guildId)
{
    ...
    if (ids.empty())
    {
        LOG_ERROR("playerbots",  "%s / %s: no rare creatures available for kill task", sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str());
        return false;
    }
    ...
    LOG_DEBUG("playerbots", "%s / %s: kill task %u", sGuildMgr->GetGuildById(guildId)->GetName().c_str(), player->GetName().c_str(), creatureId);
...
bool GuildTaskMgr::IsGuildTaskItem(uint32 itemId, uint32 guildId)
{
    ...
    QueryResult results = PlayerbotDatabase.PQuery("SELECT `value`, `time`, validIn FROM playerbot_guild_tasks WHERE `value` = '%u' AND guildid = '%u' AND `type` = 'itemTask'",
        itemId, guildId);
...
std::map<uint32, uint32> GuildTaskMgr::GetTaskValues(uint32 owner, std::string type, uint32* validIn /* = nullptr */)
{
    ...
    QueryResult results = PlayerbotDatabase.PQuery("SELECT `value`, `time`, validIn, guildid FROM playerbot_guild_tasks WHERE owner = '%u' AND `type` = '%s'",
        owner, type.c_str());
...
uint32 GuildTaskMgr::GetTaskValue(uint32 owner, uint32 guildId, std::string type, uint32* validIn /* = nullptr */)
{
    ...
    QueryResult results = PlayerbotDatabase.PQuery("SELECT `value`, `time`, validIn FROM playerbot_guild_tasks WHERE owner = '%u' AND guildid = '%u' AND `type` = '%s'",
...
{
    PlayerbotDatabase.DirectPExecute("DELETE FROM playerbot_guild_tasks WHERE owner = '%u' AND guildid = '%u' AND `type` = '%s'", owner, guildId, type.c_str());
    if (value)
    {
        PlayerbotDatabase.DirectPExecute("INSERT INTO playerbot_guild_tasks (owner, guildid, `time`, validIn, `type`, `value`) VALUES ('%u', '%u', '%u', '%u', '%s', '%u')",
...
bool GuildTaskMgr::HandleConsoleCommand(ChatHandler* handler, char const* args)
{
    ...
    if (cmd == "reset")
    {
        PlayerbotDatabase.PExecute("DELETE FROM playerbot_guild_tasks");
    ...
    if (cmd.find("stats ") != std::string::npos)
    {
        ...
        QueryResult result = PlayerbotDatabase.PQuery("SELECT `value`, `time`, validIn, guildid FROM playerbot_guild_tasks WHERE owner = '%u' AND type='activeTask' ORDER BY guildid",
            owner);
        ...
        if (result)
        {
            do
            {
                ...
                LOG_INFO("playerbots", "%s: %s valid in %s ['%s']",
    ...
    if (reward || advert)
    {
        ...
        uint32 owner = guid.GetCounter();
        QueryResult result = PlayerbotDatabase.PQuery("SELECT DISTINCT guildid FROM playerbot_guild_tasks WHERE owner = '%u'", owner);
...
bool GuildTaskMgr::CheckItemTask(uint32 itemId, uint32 obtained, Player* ownerPlayer, Player* bot, bool byMail)
{
    ...
    if (itemTask != itemId)
    {
        LOG_DEBUG("playerbots", "%s / %s: item %u is not guild task item (%u)", guild->GetName().c_str(), ownerPlayer->GetName().c_str(), itemId, itemTask);
    ...
    if (obtained >= count)
    {
        LOG_DEBUG("playerbots", "%s / %s: guild task complete", guild->GetName().c_str(), ownerPlayer->GetName().c_str());
    ...
    {
        LOG_DEBUG("playerbots", "%s / %s: guild task progress %u/%u", guild->GetName().c_str(), ownerPlayer->GetName().c_str(), obtained, count);
...
void GuildTaskMgr::CleanupAdverts()
{
    ...
    if (count > 0)
    {
        ...
        LOG_INFO("playerbots", "%d old gtask adverts removed", count);
    }
}
...
void GuildTaskMgr::RemoveDuplicatedAdverts()
{
    ...
    if (count > 0)
    {
        ...
        DeleteMail(buffer);
        LOG_INFO("playerbots", "%d duplicated gtask adverts removed", count);
    }
