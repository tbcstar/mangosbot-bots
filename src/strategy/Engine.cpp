...
void Engine::addStrategies(std::string const& first, ...)
{
	addStrategy(first);
...
void Engine::LogAction(char const* format, ...)
{
    ...
    else
    {
        ...
        LOG_INFO("playerbots",  "%s %s", bot->GetName().c_str(), buf);
    }
}
...
void Engine::LogValues()
{
    ...
    LOG_DEBUG("playerbots",  "Values for %s: %s", bot->GetName().c_str(), text.c_str());
}
