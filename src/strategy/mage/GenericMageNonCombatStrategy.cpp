...
void GenericMageNonCombatStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        "arcane intellect",
        NextAction::array(0, new NextAction("arcane intellect", 21.0f), nullptr)));

	triggers.push_back(new TriggerNode(
		"no drink",
		NextAction::array(0, new NextAction("conjure water", 16.0f), nullptr)));
...
void MageBuffStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        "give water",
        NextAction::array(0, new NextAction("give water", 14.0f), nullptr)));

    triggers.push_back(new TriggerNode(
        "give food",
        NextAction::array(0, new NextAction("give food", 13.0f), nullptr)));
}
