...
bool CustomStrategyEditAction::PrintHelp()
{
    ...
    QueryResult results = PlayerbotDatabase.PQuery("SELECT DISTINCT name FROM playerbot_custom_strategy WHERE owner = '%u'", owner);
    if (results)
    {
        do
        {
...
bool CustomStrategyEditAction::Print(std::string const& name)
{
    ...
    uint32 owner = botAI->GetBot()->GetGUID().GetCounter();
    QueryResult results = PlayerbotDatabase.PQuery("SELECT idx, action_line FROM playerbot_custom_strategy WHERE name = '%s' AND owner = '%u' ORDER BY idx",
            name.c_str(), owner);
    if (results)
    {
        do
...
{
    uint32 owner = botAI->GetBot()->GetGUID().GetCounter();
    QueryResult results = PlayerbotDatabase.PQuery("SELECT action_line FROM playerbot_custom_strategy WHERE name = '%s' AND owner = '%u' AND idx = '%u'",
        name.c_str(), owner, idx);
    if (results)
    {
        if (command.empty())
        {
            PlayerbotDatabase.DirectPExecute("DELETE FROM playerbot_custom_strategy WHERE name = '%s' AND owner = '%u' AND idx = '%u'",
                name.c_str(), owner, idx);
        }
        else
        {
            PlayerbotDatabase.DirectPExecute("UPDATE playerbot_custom_strategy SET action_line = '%s' WHERE name = '%s' AND owner = '%u' AND idx = '%u'",
                command.c_str(), name.c_str(), owner, idx);
        }
    }
    else
    {
        PlayerbotDatabase.DirectPExecute("INSERT INTO playerbot_custom_strategy (name, owner, idx, action_line) VALUES ('%s', '%u', '%u', '%s')",
