...
bool LfgAcceptAction::Execute(Event event)
{
    ...
    if (id)
    {
        ...
        //botAI->ChangeStrategy("-grind,-rpg,-travel", BOT_STATE_NON_COMBAT);
        botAI->Reset();
...
bool LfgJoinAction::isUseful()
{
    ...
    if ((botAI->GetMaster() && !botAI->GetMaster()->GetPlayerbotAI()) || bot->GetGroup() && bot->GetGroup()->GetLeaderGUID() != bot->GetGUID())
    {
        //botAI->ChangeStrategy("-lfg", BOT_STATE_NON_COMBAT);
        return false;
    }
