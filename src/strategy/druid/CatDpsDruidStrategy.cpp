...
class CatDpsDruidStrategyActionNodeFactory : public NamedObjectFactory<ActionNode>
{
    ...
    private:
        static ActionNode* faerie_fire_feral()
        {
        ...
        static ActionNode* feral_charge_cat()
        {
        ...
        static ActionNode* ferocious_bite()
        {
...
CatDpsDruidStrategy::CatDpsDruidStrategy() : FeralDruidStrategy(botAI)
{
...
void CatDpsDruidStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    triggers.push_back(new TriggerNode("faerie fire (feral)", NextAction::array(0, new NextAction("faerie fire (feral)", ACTION_HIGH + 1), nullptr)));
